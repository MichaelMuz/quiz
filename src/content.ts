...
export type CommandName = "fd" | "sed" | "xargs" | "cut" | "fzf" | "grep" | "rg" | "jq" | "awk" | "printf" | "kubectl" | "less" | "perl" | "find" | "tr" | "head" | "tail" | "ps" | "watch";
...
export const contentBank: StaticItem[] = [
  ...
  },
  {
    ...
    kind: "bash",
    topic: "Bash",
    prompt: "Initial shell transcript:\n$ cat out.txt\nOLD\n$ cat log.txt\nOLD\n\nCommands to run:\nprintf '%s\\n' FIRST >out.txt\nprintf '%s\\n' SECOND >>log.txt\n\nWhat are the exact final file contents? Each final line, including the last line in each file, is newline-terminated.",
    answer: "Both commands redirect fd 1. > opens out.txt and truncates it before printf writes, while >> opens log.txt for writing and appends instead.\n\nFinal out.txt:\nFIRST\n\nFinal log.txt:\nOLD\nSECOND\n\nAll three displayed file-content lines end with newline bytes.",
    choices: [
      "out.txt:\nFIRST\n\nlog.txt:\nOLD\nSECOND",
      "out.txt:\nOLD\nFIRST\n\nlog.txt:\nOLD\nSECOND",
      "out.txt:\nFIRST\n\nlog.txt:\nSECOND",
